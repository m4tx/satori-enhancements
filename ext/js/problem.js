(function () {
    ...
    const { contestID, problemID } = getContestAndProblemID(
    ...
    async function loadResults() {
        ...
        if (!response.ok) {
            throw Error(
                `Results request failed with HTTP status code ${response.status}`,
            );
        }
        ...
        if (content.find('> .wheel > a.wheelitem').length > 0) {
            const row = $(
                '<tr><td colspan="3" class="centered"><a class="stdlink">See more submissions</a></td></tr>',
            );
